...
class DOMEstilo(object):
    def __init__(self, caminho="dados/banco/", nomeArq="Estilos"):
    ...
    def recuperar(self, ident):
        ...
        for linha in linhas:
            ...
            if int(linha[0]) == ident:
                estilo = self.__criarEstilo(linha)
        return estilo
    ...
    def listarTodos(self):
        ...
        for linha in linhas:
            linha = linha.split(self.separador)
            listaEstilos.append(self.__criarEstilo(linha))
        return listaEstilos